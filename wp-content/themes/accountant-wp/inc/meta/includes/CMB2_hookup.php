...
class CMB2_hookup {
	...
	public static function register_scripts() {
		...
		if ( ! is_admin() ) {
			...
			wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
				'clear'         => esc_html__( 'Clear', 'accountant-wp' ),
				'defaultString' => esc_html__( 'Default', 'accountant-wp' ),
				'pick'          => esc_html__( 'Select Color', 'accountant-wp' ),
				'current'       => esc_html__( 'Current Color', 'accountant-wp' ),
			) );
		}
		...
		wp_localize_script( 'cmb2-scripts', 'cmb2_l10', apply_filters( 'cmb2_localized_data', array(
			...
			'defaults'         => array(
				...
				'date_picker'  => array(
					'changeMonth'     => true,
					'changeYear'      => true,
					'dateFormat'      => esc_html__( 'mm/dd/yy', 'accountant-wp' ),
					'dayNames'        => explode( ',', esc_html__( 'Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday', 'accountant-wp' ) ),
					'dayNamesMin'     => explode( ',', esc_html__( 'Su, Mo, Tu, We, Th, Fr, Sa', 'accountant-wp' ) ),
					'dayNamesShort'   => explode( ',', esc_html__( 'Sun, Mon, Tue, Wed, Thu, Fri, Sat', 'accountant-wp' ) ),
					'monthNames'      => explode( ',', esc_html__( 'January, February, March, April, May, June, July, August, September, October, November, December', 'accountant-wp' ) ),
					'monthNamesShort' => explode( ',', esc_html__( 'Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec', 'accountant-wp' ) ),
					'nextText'        => esc_html__( 'Next', 'accountant-wp' ),
					'prevText'        => esc_html__( 'Prev', 'accountant-wp' ),
					'currentText'     => esc_html__( 'Today', 'accountant-wp' ),
					'closeText'       => esc_html__( 'Done', 'accountant-wp' ),
					'clearText'       => esc_html__( 'Clear', 'accountant-wp' ),
				),
				'time_picker'  => array(
					'timeOnlyTitle' => esc_html__( 'Choose Time', 'accountant-wp' ),
					'timeText'      => esc_html__( 'Time', 'accountant-wp' ),
					'hourText'      => esc_html__( 'Hour', 'accountant-wp' ),
					'minuteText'    => esc_html__( 'Minute', 'accountant-wp' ),
					'secondText'    => esc_html__( 'Second', 'accountant-wp' ),
					'currentText'   => esc_html__( 'Now', 'accountant-wp' ),
					'closeText'     => esc_html__( 'Done', 'accountant-wp' ),
					'timeFormat'    => esc_html__( 'hh:mm TT', 'accountant-wp' ),
					'controlType'   => 'select',
					'stepMinute'    => 5,
				),
			),
			'strings' => array(
				'upload_file'  => esc_html__( 'Use this file', 'accountant-wp' ),
				'remove_image' => esc_html__( 'Remove Image', 'accountant-wp' ),
				'remove_file'  => esc_html__( 'Remove', 'accountant-wp' ),
				'file'         => esc_html__( 'File:', 'accountant-wp' ),
				'download'     => esc_html__( 'Download', 'accountant-wp' ),
				'check_toggle' => esc_html__( 'Select / Deselect All', 'accountant-wp' ),
			),
		) ) );
	...
	public function add_post_enctype() {
		...
		jQuery(document).ready(function(){
			$form = jQuery("#' . esc_attr($this->form_id) . '");
			if ( $form.length ) {
				$form.attr( {
